...
#define NONE error(0);
#define UNC error(1);
/*#define MATERIAL_ switch_control(1);
...
if (p_en == true){
	...
	nn += 26;
	//&names+n-1 = {'q','w','d'};
	strcpy(&names+n-1,"q   w   e   r   t   y   u   i   o   p   a   s   d   f   g   h   j   k   l   z   x   c   v   b   n   m");
	printf("%s",&names);
	}
if (p_ens == true){
	n += 1;
